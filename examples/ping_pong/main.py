...
engine.renderer.add_primitive(
    "cube", material="player_mat", parent=None, transform=None
)
engine.renderer.add_primitive("cube", material="ai_mat", parent=None, transform=None)
engine.renderer.add_primitive(
    "sphere", material="ball_mat", parent=None, transform=None
)
...
def handle_input(event):
    ...
    if hasattr(event, "type") and (
        event.type == "KEYDOWN" or (hasattr(event, "type") and event.type == "KEYDOWN")
    ):
...
def ai_and_physics_update(event):
    ...
    # Player paddle (left)
    if (
        bx <= player_paddle.position[0] + 10
        and abs(by - player_paddle.position[1]) < 60
    ):
        vx = abs(vx)
        hit_paddle = "player"
    # AI paddle (right)
...
try:
    ...
    while state["running"]:
        ...
        print(
            render_ascii(
                player_paddle.position[1],
                ai_paddle.position[1],
                ball.position[0],
                ball.position[1],
            )
        )
