...
from simplex.ecs.components import (
    ...
    CollisionComponent, InputComponent, ScoreComponent
)
from simplex.ecs.systems import (
    MovementSystem, CollisionSystem, InputSystem, ScoringSystem
)
...
def game_loop():
    ...
    try:
        while True:
            ...
            print(f"\r[CLI] Player Y:{player_pos.y:3.0f} | Ball: ({ball_pos.x:3.0f},{ball_pos.y:3.0f}) | AI Y:{ai_pos.y:3.0f} | Score: P:{scoring_system.score['player']} AI:{scoring_system.score['ai']}", end="")
