...
class Engine:
    ...
    def __init__(self, config_path: str = "examples/config.toml"):
        ...
        self.audio = Audio(event_system=self.events, resource_manager=self.resource_manager)
    ...
    def _initialize_subsystems(self):
        ...
        from .ecs.systems import MovementSystem, CollisionSystem, InputSystem, ScoringSystem
    ...
    def _handle_physics_collision(self, event):
        ...
        try:
            if isinstance(event, dict):
                if event.get('type') == 'boundary' and 'ball' in event.get('entity', '').lower():
                    ...
                    if any('ball' in str(e).lower() for e in entities) and any('paddle' in str(e).lower() for e in entities):
    ...
    def _handle_score_event(self, event):
        ...
        try:
            if isinstance(event, dict) and 'scorer' in event:
                log(f"Score Event: {event['scorer']} scored! Current score: {event.get('score', {})}", level="INFO")
