...
class Renderer(RendererInterface):
    ...
    def _initialize_opengl_backend(self):
        ...
        try:
            ...
            if self.opengl_renderer.initialized:
                ...
                log(
                    "OpenGL backend failed to initialize, falling back to debug backend",
                    level="WARNING",
                )
                self._initialize_debug_backend()
        except Exception as e:
            log(
                f"OpenGL backend failed to initialize: {e}, falling back to debug backend",
                level="WARNING",
            )
            self._initialize_debug_backend()
    ...
    def add_primitive(
        ...
        log(
            f"Added primitive: {primitive} with material: {mat_obj} (instances: {instance_count})",
            level="INFO",
        )
        return node
        ...
        try:
            if self.backend == "pygame":
                ...
                if hasattr(self, "opengl_renderer"):
                    ...
                    log(
                        "OpenGLRenderer not available, falling back to debug backend",
                        level="WARNING",
                    )
                    self._render_debug()
            else:
            ...
            if self.event_system:
                self.event_system.emit(
                    "system_error", {"system": "Renderer", "error": str(e)}
                )
    ...
    def _render_debug(self):
        ...
        def render_node(node, global_transform):
            if node.primitive:
                log(
                    f"Rendering primitive: {node.primitive} with material: {node.material} (instances: {node.instance_count}) transform: {global_transform}",
                    level="INFO",
                )
                ...
                for light in self.lights:
                    if getattr(light, "type", None) == "directional":
                        log(
                            f"Applying directional light {light.direction} to {node.primitive}",
                            level="DEBUG",
                        )
                    elif getattr(light, "type", None) == "point":
                        log(
                            f"Applying point light {light.position} to {node.primitive}",
                            level="DEBUG",
                        )
                    elif getattr(light, "type", None) == "ambient":
                        log(
                            f"Applying ambient light to {node.primitive}", level="DEBUG"
                        )
