...
def main():
    ...
    renderer = OpenGLRenderer(
        width=1024, height=768, title="Simplex Engine - Interactive Test"
    )
    ...
    try:
        while running:
            ...
            if frame_count % 180 == 0:  # Every 3 seconds at 60fps
                ...
                log(
                    f"Performance: {fps:.1f} FPS, {frame_count} frames rendered",
                    level="INFO",
                )
        ...
        log(
            f"✓ Interactive test completed: {frame_count} frames in {elapsed:.1f}s (avg {avg_fps:.1f} FPS)",
            level="INFO",
        )
