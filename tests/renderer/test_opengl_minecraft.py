...
def create_minecraft_world(size=16, height_variation=4):
    ...
    log(
        f"Generated Minecraft-like world: {size}x{size} terrain with {block_count} blocks",
        level="INFO",
    )
...
def main():
    ...
    renderer = OpenGLRenderer(
        width=1280, height=720, title="Simplex Engine - Minecraft World Test"
    )
    ...
    try:
        while running:
            ...
            if frame_count % 300 == 0:
                ...
                log(
                    f"Performance: {fps:.1f} FPS rendering {block_count} blocks",
                    level="INFO",
                )
